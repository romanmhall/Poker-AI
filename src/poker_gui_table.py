...
# GLOBAL CONSTANTS (EXPOSED FOR INSPECTOR)
...
SEAT_MAP = {
    "Hero": "Hero", 
    "AI_Boss": "Seat1",
    "Seat 1": "Seat1", "Seat 2": "Seat2", "Seat 3": "Seat3", 
    "Seat 4": "Seat4", "Seat 5": "Seat5"
}
    ...
    def record_full_snapshot(self, winner, pot, rank, board, showdown_str, player_stats_str):
        ...
        if player_stats_str:
            ...
            for p_str in all_players:
                parts = p_str.split('|')
                if len(parts) == 4: row.extend(parts)
...
# 4. SHARED DRAWING FUNCTIONS (EXPOSED)
def update_player_info(graph, ui_elements, player_obj, pos_key, is_active=True):
...
def draw_hand(graph, ui_elements, pos_key, cards=None, face_down=False):
...
def draw_board(graph, ui_elements, cards):
    ...
    def write(self, text):
        ...
        if "[CSV_DATA] HAND" in clean_text and self.recorder:
            try:
                ...
                if len(parts) >= 6:
                    self.recorder.record_full_snapshot(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5])
            except: pass
...
def run_poker_table():
    ...
    bots = []
    bots.append(SmartBot("AI_Boss"))
    for i in range(2, 6):
        bot = random.choice([CallingStationBot, ManiacBot, ConservativeBot, RandomBot])(f"Seat {i}")
        bots.append(bot)
            ...
            if any(x in text for x in ["calls", "RAISES", "posts", "buys in"]):
                ...
                for p in all_players:
                    if p.name in text:
                        ...
                        update_player_info(graph, ui_elements, p, pos)
                        break
            if any(x in text for x in ["[FLOP]","[TURN]","[RIVER]"]): draw_board(graph, ui_elements, table.community_cards)
            if len(hero.hand) == 2: draw_hand(graph, ui_elements, 'Hero', hero.hand)

        if event == '-RESET-':
            for p in all_players: update_player_info(graph, ui_elements, p, SEAT_MAP.get(p.name, "Hero"))
            draw_board(graph, ui_elements, [])
            for p in bots: draw_hand(graph, ui_elements, SEAT_MAP[p.name], face_down=True)
            draw_hand(graph, ui_elements, 'Hero', cards=[]) 
